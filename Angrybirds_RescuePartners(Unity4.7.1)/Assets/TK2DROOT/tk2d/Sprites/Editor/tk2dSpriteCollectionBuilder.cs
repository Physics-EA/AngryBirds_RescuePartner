...
public class tk2dSpriteCollectionBuilder
{
	...
	static Texture2D ProcessTexture(bool premultipliedAlpha, bool additive, bool stretchPad, Texture2D srcTex, int sx, int sy, int tw, int th, ref SCGE.SpriteLut spriteLut, int padAmount)
	{
		...
		if (numNotTransparent > 0)
		{
			...
			// Comment these lines to disable trimming
//			x0 = 0;
//			y0 = 0;
//			w1 = tw;
//			h1 = th;
		...
		for (int i = 0; i < gen.textureParams.Length; ++i)
		{
			...
			if (gen.textureParams[i].dice)
			{
				...
				for (int sx = 0; sx < srcTex.width; sx += diceUnitX)
				{
					for (int sy = 0; sy < srcTex.height; sy += diceUnitY)
					{
						...
						Texture2D dest = ProcessTexture(gen.premultipliedAlpha, gen.textureParams[i].additive, true, srcTex, sx, sy, tw, th, ref diceLut, GetPadAmount(gen, i));
						if (dest)
						{
			...
			else
			{
				...
				if (!lut.isDuplicate)
				{
					...
					Texture2D dest = ProcessTexture(gen.premultipliedAlpha, gen.textureParams[i].additive, stretchPad, currentTexture, 0, 0, currentTexture.width, currentTexture.height, ref lut, GetPadAmount(gen, i));
					if (dest == null)
					{
